...
fn tool_definitions_returns_all_tools() {
    ...
    assert!(tools.len() > 100, "expected 100+ tools, got {}", tools.len());
...
fn disabled_create_filters_create_tools() {
    ...
    for tool in &tools {
        ...
        assert!(!name.starts_with("create_"), "create tool {} should be filtered", name);
...
fn disabled_delete_filters_delete_tools() {
    ...
    for tool in &tools {
        let name = tool["name"].as_str().unwrap();
        assert!(!name.starts_with("delete_"), "delete tool {} should be filtered", name);
    }
}
...
fn disabled_update_filters_update_tools() {
    ...
    for tool in &tools {
        let name = tool["name"].as_str().unwrap();
        assert!(!name.starts_with("update_"), "update tool {} should be filtered", name);
        assert!(!name.starts_with("scale_"), "scale tool {} should be filtered", name);
        assert!(!name.starts_with("restart_"), "restart tool {} should be filtered", name);
        assert!(!name.starts_with("rollback_"), "rollback tool {} should be filtered", name);
    }
}
...
fn read_only_mode_only_has_read_tools() {
    ...
    for tool in &tools {
        ...
        assert_eq!(action, Action::Read, "tool {} should be read action, got {}", name, action);
    }
}

#[test]
fn per_resource_disable_filters_specific_tool() {
    let perms = ActionPermissions::new(false, false, false, vec!["deployment-delete".to_string()]);
    let tools = super::definitions::tool_definitions(&perms);
    let names: Vec<&str> = tools.iter().filter_map(|t| t["name"].as_str()).collect();
    assert!(!names.contains(&"delete_deployment"), "delete_deployment should be filtered");
    // Other delete tools should still be present
    assert!(names.contains(&"delete_pod"), "delete_pod should still be present");
}
...
fn all_tool_schemas_are_valid() {
    ...
    for tool in &tools {
        ...
        assert_eq!(schema["type"], "object", "tool {} schema type should be object", name);
        assert!(schema.get("properties").is_some(), "tool {} missing properties", name);
        assert_eq!(schema["additionalProperties"], false, "tool {} should have additionalProperties: false", name);
    }
}
